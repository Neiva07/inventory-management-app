CREATE TABLE `app_settings` (
	...
	FOREIGN KEY (`organization_id`) REFERENCES `organizations`(`organizations_id`) ON UPDATE no action ON DELETE set null
);
...
CREATE TABLE `sync_queue` (
	`sync_queue_id` text PRIMARY KEY NOT NULL,
	`organization_id` text NOT NULL,
...
--> statement-breakpoint
CREATE INDEX `sync_queue_status_idx` ON `sync_queue` (`status`,`next_attempt_at`);--> statement-breakpoint
CREATE INDEX `sync_queue_org_idx` ON `sync_queue` (`organization_id`);--> statement-breakpoint
CREATE INDEX `sync_queue_entity_idx` ON `sync_queue` (`table_name`,`record_id`);--> statement-breakpoint
CREATE TABLE `units` (
